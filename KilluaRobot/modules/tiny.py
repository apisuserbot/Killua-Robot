...
async def _(event):
    ...
    if not (reply and (reply.media)):
        ...
        return
    killhim = await event.reply("`Processing tiny...`")
    ik = await tbot.download_media(reply)
    im1 = Image.open("KilluRobot/resources/killua.png")
    if ik.endswith(".tgs"):
        await tbot.download_media(reply, "killua.tgs")
        os.system("lottie_convert.py ken.tgs json.json")
        json = open("json.json", "r")
        jsn = json.read()
        jsn = jsn.replace("512", "2000")
        open = ("json.json", "w").write(jsn)
        os.system("lottie_convert.py json.json killua.tgs")
        file = "ken.tgs"
        os.remove("json.json")
